		...
		<view class="box" :style="oDiv">
			<view class="ring">
				<view class="ringtext ell" @touchend="gettext(item)" :style="{transform:item.style}"
					...
					<image class="_image" :src="item.user_avatar" mode="widthFix"></image>
				</view>
			</view>
		</view>
	</view>
			...
			init(){
				...
				for (var i = 0; i < circleArr.length; i++) {
					...
					for (var j = 0; j < circleArr[i]; j++) {
						var li = {}
						li.user_name = this.list[num] ? this.list[num].user_name : "";
						li.user_avatar = this.list[num] ? this.list[num].user_avatar : "";
						li.id = this.list[num] ? this.list[num].id : 0;
						num++;
