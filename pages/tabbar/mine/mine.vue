<template>
  ...
  >
    <view :class="'mini-page-body tabbar ' + global__platform__">
      <view
        :class="'panel-overlay ' + (showLeftPanel ? 'active' : '')"
        @tap.stop.prevent="leftPanelOpen(false)"
        @touchmove.stop.prevent="miniMoveHandle"
      ></view>
      <view
        :class="'sidebar flex-column ' + (showLeftPanel ? 'active' : '')"
        @touchmove.stop.prevent="miniMoveHandle"
      >
        <scroll-view class="_scroll-view" :scroll-y="true">
          <view class="list flex-column">
            <block v-for="(item, index) in myUserInfo.leftPanel" :key="index">
              <view
                class="item flex-row"
                @tap.stop.prevent="leftUrlTap(item.target_type, item.target_id)"
              >
        ...
        <view class="icons flex-row">
          <view
            ...
            @tap.stop.prevent="leftUrlTap(6, '/pagesA/mine/settings/settings')"
          >
            <view class="icon">
          ...
          <view
            class="item flex-column"
            @tap.stop.prevent="
              leftUrlTap(6, '/pages/help/feedbackIndex/feedbackIndex')
            "
          >
            <view class="icon">
      ...
      <view class="content">
        <navBar
          ...
          :background="
            'rgba(' +
            myUserInfo.user_background_color +
            ', ' +
            navbarTrans +
            ');'
          "
          :color="'rgba(255, 255, 255, ' + navbarTrans + ')'"
          :scene="5"
          :placeholder="false"
        >
          <view slot="left" class="lxy-nav-bar__logo">
            <i
              class="mini-icon mini-gengduo1"
              :style="'color: ' + iconTheme + ';'"
              @tap.stop.prevent="leftPanelOpen(true)"
            ></i>
          </view>
          <view class="img-title-center" slot="center">
            <view class="lxy-nav-bar__logo">
              <image
                class="_image"
                :src="myUserInfo.user_avatar"
                :style="'opacity: ' + navbarTrans + ';'"
              />
            </view>
          </view>
          <!-- #ifdef APP || H5 -->
          <view
            slot="right"
            class="lxy-nav-bar__logo"
            v-if="
              config_page_mine_header.right &&
              (config_page_mine_header.right.icon ||
                config_page_mine_header.right.img)
            "
          >
            <block v-if="config_page_mine_header.right.img">
              <image
                class="_image"
                :src="config_page_mine_header.right.img"
                mode=""
              ></image>
            </block>
            <block v-else>
              <i
                :class="'mini-icon' + ' ' + config_page_mine_header.right.icon"
                :style="'color: ' + iconTheme + ';'"
                @tap.stop.prevent="
                  toSliderUrl(
                    config_page_mine_header.right.type,
                    config_page_mine_header.right.target_id
                  )
                "
              ></i>
            </block>
        ...
        <view
          :style="
            'position: fixed;top: ' +
            CustomBar +
            'px;height: 16rpx;width: 750rpx;background-color: rgba(' +
            myUserInfo.user_background_color +
            ', ' +
            navbarTrans +
            ');z-index: 9;'
          "
          v-if="navbarTrans >= 0.99"
        ></view>
        <view class="user-background scene_5">
          <image
            ...
            mode="aspectFill"
            :src="
              myUserInfo.user_background_maps ||
          ...
          <view
            class="gradient"
            :style="
              'background-image: linear-gradient(180deg,rgba(0,0,0,.4),rgba(' +
              myUserInfo.user_background_color +
              ',.7) 30%,rgba(' +
              myUserInfo.user_background_color +
              ',.95) 65%,rgba(' +
            ...
            <view class="inner">
              <view class="header-top" v-if="myUserInfo && myUserInfo.id > 0">
                <view
                  :class="
                    'own-header-img-div ' +
                    (myUserInfo.avatar_frame_url ? 'has-frame' : '')
                  "
                  @tap.stop.prevent="
                    toSliderUrl(6, '/pagesA/mine/editmine/editmine')
                  "
                >
                  <image
                    class="own-header-img"
                    :lazyLoad="true"
                    mode="aspectFill"
                    :src="myUserInfo.user_avatar"
                  ></image>
                  <image
                    class="own-header-img-frame"
                    v-if="myUserInfo.avatar_frame_url"
                    :data-src="myUserInfo.avatar_frame_url"
                    :lazyLoad="true"
                    :src="myUserInfo.avatar_frame_url"
                  ></image>
                  <i
                    class="avatar-authentication-icon mini-icon mini-verify"
                    v-if="myUserInfo.is_authentication == 1"
                    @tap.stop.prevent="popupShowTap(1)"
                  ></i>
                  <i
                    class="avatar-authentication-icon mini-icon mini-jiahao-"
                    v-else
                    @tap.stop.prevent="
                      toSliderUrl(6, '/pagesA/mine/certification/certification')
                    "
                  ></i>
                </view>
                <view class="header-middle">
                  <view class="user-name-box">
                    <text :user-select="true" class="name-text">{{
                      myUserInfo.user_name
                    }}</text>
                    <i
                      class="header-labels official-icon mini-icon mini-renzhengguanli1"
                      v-if="myUserInfo.is_official > 0"
                      @tap.stop.prevent="popupShowTap(0)"
                    ></i>
                    <block v-if="config_pays && config_pays.paycode_16 == true">
                      <i
                        class="header-labels vip-icon mini-icon mini-vip"
                        v-if="myUserInfo.is_member == 1"
                        data-type="2"
                        @tap.stop.prevent="
                          toSliderUrl(6, '/pagesA/mine/members/members')
                        "
                      ></i>
                      <i
                        class="header-labels not-vip-icon mini-icon mini-vip"
                        v-if="myUserInfo.is_member != 1"
                        @tap.stop.prevent="toVipTap"
                      ></i>
                    </block>
                    <i
                      :class="
                        'header-labels lv-icon mini-icon mini-lv' +
                        myUserInfo.lv
                      "
                      data-type="0"
                    ></i>
                  ...
                  <view class="user-name-sub-box">
                    <view class="user-account-box">
                      <block v-if="config_app_multi != 0">
                        <view
                          class="change-tenant-btn flex-row"
                          @tap.stop.prevent="
                            toSliderUrl(6, '/pages/common/tenants')
                          "
                        >
                        ...
                        <view
                          class="uid"
                          @tap.stop.prevent="copyUserID(myUserInfo.id)"
                        >
                          UID：{{ myUserInfo.id }}
                        </view>
                      </block>
                      <block
                        v-else-if="
                          myUserInfo.authentication &&
                          myUserInfo.authentication.id > 0
                        "
                      >
                        <view class="auth-name">{{
                          myUserInfo.authentication.name
                        }}</view>
                        <i class="mini-icon mini-shuxian"></i>
                        <view
                          class="uid"
                      ...
                      <i
                        class="mini-icon mini-31erweima"
                        @tap.stop.prevent="toSliderUrl(12, 0)"
                      ></i>
                    </view>
                  </view>
                </view>
              ...
              <view
                class="user_state_not_zero_tip"
                v-if="
                  myUserInfo.user_state != undefined &&
                  myUserInfo.user_state != 0
                "
              >
                <i class="mini-icon mini-jinggao"></i>注意：{{
                  myUserInfo.state_tip
                }}
              </view>
              <view class="header-abstract">{{
                myUserInfo.user_introduce
              }}</view>
              <view
                class="header-user-labels"
                v-if="
                  myUserInfo.user_labels && myUserInfo.user_labels.length > 0
                "
              >
                <scroll-view
                  class="_scroll-view-label"
                  :scroll-x="true"
                  :enable-flex="true"
                >
                  <view class="label-inner">
                    <block>
                      <block v-if="myUserInfo.gender > 0">
                        ...
                        >
                          <i
                            ...
                            :style="
                              myUserInfo.age && myUserInfo.age.length > 0
                                ? 'margin-right: 8rpx;'
                                : ''
                            "
                      ...
                      <block
                        v-else-if="myUserInfo.age && myUserInfo.age.length > 0"
                      >
                        <view
                          class="label"
                          @tap.stop.prevent="
                    ...
                    <block
                      v-if="
                        ...
                        myUserInfo.university.length > 0
                      "
                    >
                      <view class="label">
                        {{ myUserInfo.university }}
                    ...
                    <view
                      class="label"
                      v-for="(label, index) in myUserInfo.user_labels"
                      :key="index"
                      @tap.stop.prevent="
                        toSliderUrl(6, '/pagesA/mine/editmine/editmine')
                      "
              ...
              <view class="header-bottom">
                <view class="header-zuo">
                  <view
                    ...
                    @tap.stop.prevent="
                      toSliderUrl(6, '/pagesA/mine/follow/follow?id=0')
                    "
                  >
                    <view class="own-header-num">
                      {{
                        myUserInfo.follow_count > 999
                          ? filters.toFix(myUserInfo.follow_count)
                          : myUserInfo.follow_count || 0
                      }}
                    </view>
                  ...
                  <view
                    class="header-bottom-li"
                    @tap.stop.prevent="
                      toSliderUrl(6, '/pagesA/mine/follow/follow?id=1')
                    "
                  >
                    <view class="own-header-num">
                      {{
                        myUserInfo.follow_user_count > 999
                          ? filters.toFix(myUserInfo.follow_user_count)
                          : myUserInfo.follow_user_count || 0
                      }}
                    </view>
                  ...
                  <view
                    class="header-bottom-li"
                    @tap.stop.prevent="popupShowTap(3)"
                  >
                    <view class="own-header-num">
                      {{
                        myUserInfo.like_count > 999
                          ? filters.toFix(myUserInfo.like_count)
                          : myUserInfo.like_count || 0
                      }}
                    </view>
                ...
                <view class="header-right-box">
                  ...
                  <view
                    ...
                    @tap.stop.prevent="
                      toSliderUrl(6, '/pagesA/mine/settings/settings')
                    "
                  >
                    <i class="mini-icon mini-shezhi2"></i>
                  </view>
                ...
                <view class="my-grid-box">
                  <scroll-view :scroll-x="true" class="_scroll-view">
                    <view class="grid-box-inner">
                      <view
                        class="grid-item"
                        v-for="(item, index) in myUserInfo.grid"
                        :key="index"
                        @tap.stop.prevent="toSliderUrl(6, item.path)"
                        :style="
        ...
        <view class="mine-tools">
          <u-sticky :offsetTop="0">
            <view class="nav">
              <view
                :class="
                  'nav-inner ' +
                  (config_audit_is_reward ? '' : 'hidden-exceptionaled')
                "
              >
                <block v-for="(item, index) in taga" :key="index">
                  <view
                    @tap.stop.prevent="select(item.name)"
                    :class="'tag-box type_' + item.name"
                  >
                    <view
                      :class="
                        'taga-item ' +
          ...
          <view class="body-box">
            ...
            <ls-skeleton
              ...
              :animate="true"
              :scene="user_list_waterfall ? 90734 : 5"
            >
              <block name="circle">
                <block v-if="user_list_waterfall">
                  <l-water-flow
                    comName="product"
                    :waterData="postWaterData"
                    ref="productwater"
                    :column-gap="8"
                    :scene="5"
                  />
                        ...
                        <block v-else>
                          <view class="topic-panel" :id="'media_' + item.id">
                            <view class="user-info-view">
                              ...
                              >
                                <view class="user-rigth" slot="right">
                                  <block
                                    v-if="
                                      config_app_mode != 'examine' &&
                                      config_pays &&
                                      config_pays.paycode_5
                                    "
                                  >
                                    <view
                                      @tap.stop.prevent="
                                        toSliderUrl(
                                          6,
                                          '/pagesC/promotion/post?id=' + item.id
                                        )
                                      "
                                      class="follow-view folled water-mine-promotion"
                                    >
                                      推广
                                    </view>
                                  </block>
                                  <block v-else>
                                    <block
                                      v-if="
                                        item.fields && item.fields.contact_phone
                                      "
                                    >
                                      <view
                                        @tap.stop.prevent="
                                      ...
                                      >
                                        <i
                                          class="mini-icon mini-dianhua inline"
                                        ></i>
                                        拨打
                                      </view>
                                    </block>
                            ...
                            <post-content
                              ...
                              @pSelectVote="selectVote"
                            ></post-content>
                            <post-footer
                              @tapShare="tapShare"
                              @taplikes="taplikes"
                              @tapComment="tapComments"
                              @tapCollect="tapCollect"
                              @tapReward="tapReward"
                              @tapGiveCoin="tapGiveCoin"
                              :words="item"
                              :index="index"
                            ></post-footer>
                            ...
                            <view
                              class="reward-box"
                              v-if="
                                item.exceptional && item.exceptional.length > 0
                              "
                              :data-id="item.id"
                            ...
                            >
                              <view class="inner">
                                ...
                                <view class="cu-avatar-text"
                                  >{{ item.exceptional_count }}次充电</view
                                >
                              </view>
                            </view>
                          </view>
                ...
                <u-popup :show="rewardPopup" @close="shutReward">
                  <view class="reward-add-box">
                    <image
                      class="reward-add-box-image"
                      :src="config_img.reward_select"
                      mode="aspectFill"
                    />
                    ...
                    <view class="reward-add-box-view">
                      <view
                        @tap.stop.prevent="addRewardPrice"
                ...
                <u-popup
                  :show="payContentPopup"
                  @close="shutPayContent"
                  :zIndex="99999991009"
                >
                  <block v-if="payContentObj && payContentObj.price > 0">
                    <view class="pay_content_box">
                      ...
                      <view class="pay_for_item rest">
                        ...
                        <view class="pay_for_item_num"
                          >{{ payContentObj.rest }}%</view
                        >
                      </view>
                      <view class="pay_for_item has_hidden">
                        <text class="_text">隐藏内容：</text>
                        <view class="pay_for_item_num"
                          >{{ payContentObj.has_hidden }}字符</view
                        >
                      </view>
                      <view class="pay_for_item has_file">
                        <text class="_text">隐藏图片：</text>
                        <view class="pay_for_item_num">{{
                          payContentObj.has_img
                        }}</view>
                      </view>
                      <view class="pay_for_item has_file">
                        <text class="_text">隐藏视频：</text>
                        <view class="pay_for_item_num">{{
                          payContentObj.has_video
                        }}</view>
                      </view>
                      <view class="pay_for_item has_file">
                        <text class="_text">隐藏附件：</text>
                        <view class="pay_for_item_num">{{
                          payContentObj.has_file
                        }}</view>
                      </view>
                      <view class="buy_button" @tap.stop.prevent="payPost"
                        >立即购买</view
                      >
                      <view class="tips"
                        >虚拟物品存在可复制性，请谨慎购买，不可退货，若不慎被骗，请立即举报并说明原因</view
                      >
                    </view>
                  </block>
                </u-popup>
                  ...
                  <view class="dialog_container">
                    ...
                    >
                      <view class="dialog_conent">
                        <view
                          class="reward-user-box"
                          v-for="(item, index) in exceptionalList"
                          :key="item.rindex"
                        >
                          <view
                            class="reward-user-avatar-view"
                            hover-class="none"
                            @tap.stop.prevent="toSliderUrl(4, item.user.id)"
                          >
                            <image
                              class="reward-user-avatar-image"
                              compression="avatar"
                              mode="aspectFill"
                ...
                <view :class="'dialog ' + (bounced ? 'dialog_show' : '')">
                  ...
                  <view class="dialog_container">
                    <view class="dialog_conent">
                      <view
                        @tap.stop.prevent="bouncedDeleteTap"
                        class="delete-btn"
                        id="delete"
                        >确认删除
                      </view>
                      <view @tap.stop.prevent="bouncedTap" class="cancel-btn"
                        >取消</view
                      >
                    </view>
                  </view>
                  ...
                  <view class="dialog_container">
                    <view class="dialog_conent">
                      <view class="dialog_box">
                        ...
                        <view
                          @tap.stop.prevent="sharePosterClick"
                          class="dialog_btn"
                        >
                      ...
                      <view @tap.stop.prevent="onClickShare" class="cancel-btn"
                        >取消</view
                      >
                    </view>
                  </view>
                ...
                <view :class="showComments ? 'dialog_show' : ''">
                  ...
                  <view class="dialog_container">
                    ...
                    <comment-scroll
                      unicom-id="commentScroll_5_0"
                      ref="commentScroll"
                      :postsId="postsId"
                      @tapComment="tapComment"
                    ></comment-scroll>
                    <comment-form
                      unicom-id="commentForm_5_0"
                      ref="commentForm"
                      :postsId="postsId"
                      :commentFormShow="commentFormShow"
                      @maskClick="commentFormMaskClick"
                      @sendComment="onInputComment"
                      @tapComment="tapComment"
                      :placeholder="replyName || '此时此刻想说~'"
                    ></comment-form>
                  </view>
                </view>
              </block>
              <view class="empty-box" v-if="isNul">
                ...
                <block v-else>
                  <view
                    class="bon-box"
                    @tap.stop.prevent="toSliderUrl(-4, null)"
                    v-if="currentItem == 'post'"
                    >立即登录</view
                  >
                </block>
            ...
            <u-loadmore
              v-if="loadmoreShow"
              :status="isLastPage ? 'nomore' : 'loading'"
              nomoreText="── THE END ──"
              iconSize="28"
              fontSize="28"
              color="#989898"
      ...
      <u-popup :show="imageShow" @close="popupShowTap(-1)">
        <image
          ...
          v-if="typeShow == 0"
          class="vip-img"
          :src="config_user_official_popup || ''"
          mode="aspectFill"
        ></image>
        <image
          @tap.stop.prevent="popupShowTap(-1)"
          v-if="typeShow == 1"
          class="vip-img"
          :src="config_user_authentication_popup"
          mode="aspectFill"
        ></image>
        <image
          @tap.stop.prevent="popupShowTap(-1)"
      ...
      <post-share
        unicom-id="postShare_5_0"
        :postsId="postsId"
        :show="showSharePopup"
        @postfeedback="postFeedback"
      ></post-share>
      <hover-ball v-if="isOfficial > 1" /><my-toast
        unicom-id="mytotast_5_0"
        ref="mytoast"
        :isdistance="true"
        :toast="mytoast_data"
        @giveCoin="onInputComment"
        @postfeedback="postFeedback"
      ></my-toast>
    </view>
  </view>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      user_list_waterfall: {
        get() {
          ...
          if (
            that.myUserInfo.setting &&
            that.myUserInfo.setting.diy_scene_8_post_list_style != 1
          ) {
      ...
      config_app_mode: {
        get() {
          let that = this
          if (
            that.$store.state.config.app &&
            that.$store.state.config.app.mode
          ) {
    ...
    onLoad: function (options) {
      ...
      console.log(page.route, '0000000000000')
      let that = this
    ...
    onShow: function () {
      ...
      // #endif
      //  #ifndef MP
      that.$store.commit('updateTabbarIndex', 4)
      // #endif
      ...
      if (
        that.myUserInfo &&
        that.myUserInfo.timeStamp > 0 &&
        that.myUserInfo.timeStamp > that.lastRefrshTimeStamp
      ) {
    ...
    onShareAppMessage: function (res) {
      ...
      if (res.from == 'button') {
        ...
        share_title = share_title
          .replace('[user_name]', that.$store.state.myUserInfo.user_name)
          .replace(
            '[user_introduce]',
        ...
        return {
          title: share_title,
          path:
            '/pages/tabbar/mine/mine?id=' +
            that.$store.state.myUserInfo.id +
            '&fph=1&from_user=' +
            that.$store.state.userInfo.id +
            '&tenant=' +
            that.global__tenant__.tenant_id,
    ...
    onShareTimeline: function (res) {
      let that = this
      let my_avater_img = that.$store.state.myUserInfo
        ? that.$store.state.myUserInfo.user_avatar
        : ''
      let my_name = that.$store.state.myUserInfo
        ? that.$store.state.myUserInfo.user_name
        : ''
      let my_user_id = that.$store.state.myUserInfo
        ? that.$store.state.myUserInfo.id
        : ''
      let share_title = that.$store.state.config.share.format.app_share_user
      share_title = share_title
        .replace('[user_name]', my_name)
        .replace(
          '[user_introduce]',
          that.$store.state.myUserInfo.user_introduce
        )
      return {
        ...
        query:
          ...
          that.global__tenant__.tenant_id,
        imageUrl:
          uni.wen.util.removeStyles(
            my_avater_img || that.$store.state.config.img.default_share
          ) +
          '?imageMogr2/thumbnail/200x200/crop/200x200/gravity/center/format/jpg/interlace/0/quality/100'
      }
    },
      ...
      shutLogin() {
        ...
        that.$store.commit('updateUserInfo', {
          ...
          leftPanel: [
            ...
            {
              icon: 'mini-icon2 mini-zhifumima',
            ...
            {
              icon: 'mini-icon2 mini-xiaofeijilu',
            ...
            {
              icon: 'mini-icon2 mini-shouhuodizhi',
            ...
            {
              icon: 'mini-icon2 mini-woderenzheng',
            ...
            {
              icon: 'mini-icon2 mini-wodequanzi',
