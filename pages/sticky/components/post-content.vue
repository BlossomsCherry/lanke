			...
			<block v-if="scene == 9">
				<view @longpress.stop.prevent="stopPrevent">
					<view v-if="words.is_only_text == 1">
						<text :class="'content-view ' + (is_ellipsis ? 'ellipsis' : 'unellipsis')">
			...
			<view class="inner media_type_1">
				...
				<block v-if="config_app_mode != 'examine'">
					<view :class="'videomodel ' + (words.video.show_type == 0 ? 'h-video' : 'w-video')"
						...
						<yingbing-video ref="wordVideo" :src="words.video.video_url" :style="'height:'+words.video.display_height+'rpx;width: 710rpx;'" :poster="words.video.video_thumb_url" v-if="words.video.show_type != 0" @fullscreenChange="fullscreen" @play="mpvideoPlay"></yingbing-video>
						<yingbing-video ref="wordVideo" :src="words.video.video_url" :style="'height:'+words.video.display_height+'rpx;width: 710rpx;'" :poster="words.video.video_thumb_url" v-else @fullscreenChange="fullscreen" @play="mpvideoPlay"></yingbing-video>
						<!-- #endif -->
					
						<!-- #ifdef MP -->
						<view class="video-height">
							<video :id="'wordVideo-'+words.video.id" class="mpvideo video-image z-i4" @tap.stop.prevent="mpvideoPlay" @ended="()=>{isplaying = false}" @pause="()=>{isplaying = false}" 
			...
			<block v-if="config_app_mode != 'examine'">
				<view class="media_type_3">
					<view :class="'videomodel ' + (words.video.show_type == 0 ? 'h-video' : 'w-video')"
						v-if="words.video != null && words.video != '' && words.post_type != 'single'" @tap.stop.prevent="stopPrevent">
						<!-- #ifndef MP -->
						<yingbing-video ref="wordVideo" :src="words.video.video_url" :style="'height:'+ words.video.display_height +'rpx;width: 750rpx;'" :poster="words.video.video_thumb_url" v-if="words.video.show_type != 0" @fullscreenChange="fullscreen" @play="mpvideoPlay"></yingbing-video>
						<yingbing-video ref="wordVideo" :src="words.video.video_url" :style="'height:'+ words.video.display_height +'rpx;width: 750rpx;'" :poster="words.video.video_thumb_url" v-else @fullscreenChange="fullscreen" @play="mpvideoPlay"></yingbing-video>
						...
						<!-- #ifdef MP -->
						<view class="video-height">
							<video :id="'wordVideo-'+words.video.id" class="mpvideo video-image z-i4" @play.stop.prevent="mpvideoPlay" @ended="()=>{isplaying = false}" @pause="()=>{isplaying = false}" 
								:unit-id="vutad" :controls="true" :show-fullscreen-btn="true"
								:enable-play-gesture="true" :show-play-btn="true" :poster-for-crawler="words.video.video_thumb_url" :poster="words.video.video_thumb_url"
			...
			<view class="inner media_type_1">
				...
				<block v-if="config_app_mode != 'examine'">
					<view :class="'videomodel ' + (words.refer.video.show_type == 0 ? 'h-video' : 'w-video')"
						...
						<!-- #ifndef MP -->
						<yingbing-video ref="referVideo" :src="words.refer.video.video_url" :style="'height:'+ words.refer.video.display_height +'rpx;width: 750rpx;'" :poster="words.refer.video.video_thumb_url" v-if="words.refer.video.show_type != 0" @fullscreenChange="fullscreen" @play="referMpvideoPlay"></yingbing-video>
						<yingbing-video  ref="referVideo" :src="words.refer.video.video_url" :style="'height:'+ words.refer.video.display_height +'rpx;width: 750rpx;'" :poster="words.refer.video.video_thumb_url" v-else @fullscreenChange="fullscreen" @play="referMpvideoPlay"></yingbing-video>
						...
						<!-- #ifdef MP -->
						<view class="video-height">
							<video :id="'referVideo'+words.refer.video.id" class="mpvideo video-image z-i4" @play.stop.prevent="referMpvideoPlay" @ended="()=>{referIsplaying = false}" @pause="()=>{referIsplaying = false}" 
								:unit-id="vutad" :controls="true" :show-fullscreen-btn="true"
		...
		<view class="inner">
			...
			<block v-if="words.bureau_list && words.bureau_list.length > 0">
				<view class="bureau-list-view">
					<swiper :indicator-dots="words.bureau_list.length > 1" :autoplay="true" :interval="3000" :duration="1000" class="bureau-swiper">
						<block  v-for="(item, index) in words.bureau_list" :key="index">
							<swiper-item>
								<view class="list_box" @tap.stop.prevent="toSliderUrl(34, item.id)">
									<view class="list_box_cont">
										<image class="image" :src="item.pic"
											mode="aspectFill"></image>
										<view class="cont">
											...
											<view class="other-text-wrapper">
											  ...
											  <view class="time">{{ item.service_date_start + ' ' + item.service_time_start }}</view>
											</view>
											
											<view class="other-text-wrapper">
											  <u-icon name="map" size="25" color="A3A3A3"/>
											  <view class="address">{{ item.distance }} {{ item.address_detailed }}</view>
											</view>
										</view>
									</view>
								</view>
				...
				if(that.isplaying || that.referIsplaying){
					...
					if(that.isplaying){
						if(that.wordVideoContext === null){
							uni.createSelectorQuery().in(this).select('#wordVideo-' + that.words.video.id).context(res => {
							  if (res) {
					...
					if(that.referIsplaying){
						if(that.referVideoContext === null){
							uni.createSelectorQuery().in(this).select('#referVideo' + that.words.refer.video.id).context(res => {
							  if (res) {
