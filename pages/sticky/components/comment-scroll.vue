	...
	<ls-skeleton :loading="commentSkeletonLoad" :skeleton="commentListSkeleton">
		<scroll-view :show-scrollbar="false" :scroll-y="scene != 9" class="dialog_comments" enhanced  @scrolltolower="loadRefreshComments(false)">
			<view class="comment-avatar-quick" @tap.stop.prevent="tapComment('', '', '')" v-if="scene == 9">
				<view class="left">
					<image class="_image" :src="myUserInfo.user_avatar || config_img.default_avatar" mode=""></image>
				</view>
				<view class="right">
					<view class="u-line-1">说点什么...</view>
					<view>
						<i class="mini-icon mini-a-"></i>
						<i class="mini-icon mini-biaoqing1"></i>
						<i class="mini-icon mini-tupian21"></i>
					</view>
				</view>
			</view>
					...
					<view class="comment-header">
						<view class="comment-header-left" @tap.stop.prevent="toSliderUrl(4, item.user.id)">
							...
							<view class="comment-user-date">
								...
								<block v-else>
									<i class="comment-header-label gender-m-icon mini-icon mini-xingbienan" v-if="item.user.gender == 1"></i>
									<i class="comment-header-label gender-w-icon mini-icon mini-xingbienv" v-if="item.user.gender == 2"></i>
									<i class="comment-header-label official-icon mini-icon mini-renzhengguanli1" v-if="item.user.is_official == 1"></i>
									<i class="comment-header-label vip-icon mini-icon mini-vip" v-if="item.user.is_member == 1"></i>
									<i :class="'comment-header-label lv-icon mini-icon mini-lv' + item.user.lv"></i>
								</block>
							</view>
						</view>
						<view class="comment-header-right">
							<i class="mini-icon mini-xihuan-shixin" @tap.stop.prevent="tapCommentlike(cindex, item.id)" v-if="item.is_like"></i>
							<i class="mini-icon mini-xihuan-kongxin" @tap.stop.prevent="tapCommentlike(cindex, item.id)" v-else></i>
							...
							<contextualPopup subject="comment" :popList="item.allowable" :para="{ type: 'parent-comment' ,  index: cindex}">
							  <view class="contentinfo" slot="content">
							  	<block v-if="item.comment_content != null">
									<block v-if="item.is_only_text == 1">
										<text>
											<text class="icon-text isSticky-icon" v-if="item.is_sticky"></text>
											{{ item.comment_content }}
											<text class="format_time">{{ item.format_time + '&nbsp;&nbsp;' + item.ip_address }}</text>
										</text>
									</block>
									<block v-else>
										<mp-html :isSticky="item.is_sticky" :content="item.comment_content" :scene="scene" :loading-img="config_img.default">
											<!-- #ifndef APP-NVUE -->
							...
							<view class="comment-header sub">
								<view class="comment-header-left" @tap.stop.prevent="toSliderUrl(4, items.user.id)">
									<view class="comment-avatar-wrap sub">
										...
										<i class="avatar-authentication-icon mini-icon mini-verify" v-if="items.user.is_authentication == 1"></i>
									</view>
									<view class="comment-user-date">
										...
										<block v-else>
											<i class="comment-header-label gender-m-icon mini-icon mini-xingbienan" v-if="items.user.gender == 1"></i>
											<i class="comment-header-label gender-w-icon mini-icon mini-xingbienv" v-if="items.user.gender == 2"></i>
											<i class="comment-header-label official-icon mini-icon mini-renzhengguanli1" v-if="items.user.is_official == 1"></i>
											<i class="comment-header-label vip-icon mini-icon mini-vip" v-if="items.user.is_member == 1"></i>
											<i :class="'comment-header-label lv-icon mini-icon mini-lv' + items.user.lv"></i>
										</block>
									</view>
								</view>
								<view class="comment-header-right" @tap.stop.prevent="tapCommentslike(cindex, cindexs, items.id)">
									<i class="mini-icon mini-xihuan-shixin"  v-if="items.is_like"></i>
									<i class="mini-icon mini-xihuan-kongxin" v-else></i>
									<!-- #ifndef APP-NVUE -->
									...
									<contextualPopup subject="comment" :popList="items.allowable"  :para="{ type: 'sub-comment' , pindex: cindex, index: cindexs}">
										<view class="contentinfo" slot="content">
											<block v-if="items.comment_content != null">
												<block v-if="items.is_only_text == 1">
													<text>
														<text class="content-before"  v-if="item.user_id != items.comment_agent_id">回复<text class="_text">{{ items.comment_agent_name }}</text>：</text>
														{{ items.comment_content }}
														<text class="content-after format_time">{{ items.format_time + '&nbsp;&nbsp;' + items.ip_address }}</text>
													</text>
												</block>
												<block v-else>
													<mp-html :content="items.comment_content" :scene="scene" :loading-img="config_img.default">
														<view slot="content-before" class="content-before" v-if="item.user_id != items.comment_agent_id">回复<text class="_text">{{ items.comment_agent_name }}</text>：</view>
														<!-- #ifndef APP-NVUE -->
						...
						<view class="replay-more" v-if="!item.child_is_last_page" @tap.stop.prevent="getCommentByCommentId(item.id, cindex)">
							<block v-if="item.child_loading === true">
								<view aria-label="Loading..." role="status" class="spin-loader">
								  <i class="icon mini-icon mini-jiazai"></i>
			...
			<block v-if="commentIsNull && comments.length <= 0">
				<u-empty width="300" height="300" text="快来抢占沙发吧" textSize="28"/>
			</block>
	...
	methods: {
		...
		loadRefreshComments(refresh){
			...
			if(refresh === true){
				...
				if(that.isCommentEndPage || that.comments.length <= 0){
					return false;
				}
			...
			uni.wen.util.request(uni.wen.api.ApiRootUrl + 'comment/byPostsId', {
				...
				if(res.status){
					...
					if(res.data.current_page != that.cPage){
						return false;
					}
