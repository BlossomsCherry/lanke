...
const getMessages = function () {
  let that = this
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/info')
    .then(function (res) {
      ...
      if (
        res.data.common.client_id == null ||
        res.data.common.client_id == undefined
      ) {
...
const getDetailsMessages = function (type) {
  ...
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/getDetailsMessages', {
      type: type,
      page: that.page
    })
    .then(function (res) {
...
const readMessages = function (type) {
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/readMessages', {
      type: type
    })
    .then(function (res) {})
}
...
const toAddChat = function () {
  ...
  if (
    (typeof chat_content == 'undefined' ||
      chat_content == null ||
      chat_content.trim() == '') &&
    (that.chat_image == '' || that.chat_image === undefined) &&
    (that.chat_audio_url == '' || that.chat_audio_length == 0)
  ) {
    uni.showToast({
      title: '你还没有讲话昵',
      icon: 'none'
...
const toAddChatImage = function (type) {
  ...
  let _toAddChatImage = type => {
    uni.chooseImage({
      ...
      success(res) {
        ...
        uni.wen.util
          .uploadFile(
            uni.wen.api.ApiRootUrl + 'files/uploads',
            tempFilePaths[0],
            15
          )
          .then(function (res) {
  ...
  uni.wen.util.checkAppPermission(
    type == 'camera' ? 'camera' : 'photoLibrary-write',
    type == 'camera'
      ? '[摄像头权限]获取您此刻所拍的照片，用于发送图片私信'
      : '[相册读写权限]获取您的相册图片，用于发送图片私信',
    () => {
      _toAddChatImage(type)
    },
    that
...
const toAddChatAudio = function (msg) {
  ...
  uni.wen.util
    .uploadFile(uni.wen.api.ApiRootUrl + 'files/uploads', msg.url, 15)
    .then(function (res) {
      if (res.status) {
        that.setData({
...
const getUserChat = function (type) {
  ...
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/getUserChat', {
      oid: that.oid,
      page: that.page
    })
    .then(function (res) {
...
const getUserChatList = function () {
  let that = this
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/getUserChatList')
    .then(function (res) {
      uni.loading(false)
      if (res.status) {
        let args = {}
...
const readUserChat = function () {
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/readUserChat', {
      oid: this.oid
    })
    .then(function (res) {})
} //删除聊天记录
...
const delMessage = function (e) {
  let that = this
  uni.wen.util
    .request(uni.wen.api.ApiRootUrl + 'massages/userDelMessage', {
