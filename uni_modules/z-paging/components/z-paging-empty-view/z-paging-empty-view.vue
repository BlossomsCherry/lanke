...
<template>
	<view :class="{'zp-container':true,'zp-container-fixed':emptyViewFixed}" :style="[finalEmptyViewStyle]" @click="emptyViewClick">
		<view class="zp-main">
			<image v-if="!emptyViewImg.length" class="zp-main-image" mode="heightFix" :style="[emptyViewImgStyle]" :src="emptyImg" />
			<image v-else class="zp-main-image" mode="heightFix" :style="[emptyViewImgStyle]" :src="emptyViewImg" />
...
<script>
	...
	export default {
		...
		props: {
			...
			emptyViewImg: {
				type: String,
				default: uni.$store.state?.config?.img?.empty ?? ''
			},
			//是否显示空数据图重新加载按钮
			showEmptyViewReload: {
				type: Boolean,
		...
		computed: {
			emptyImg() {
                return this.isLoadFailed ? zStatic.base64Error : (uni.$store.state?.config?.img?.empty ?? zStatic.base64Empty);
			},
			finalEmptyViewStyle(){
