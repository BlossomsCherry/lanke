export const commentItem = {
	...
	methods: {
		...
		getCommentList() {
			...
			uni.wen.util.request(uni.wen.api.ApiRootUrl + 'pets/comment/list', {
				pet_adoption_id: that.id,
				page: that.page,
			}).then(function(res) {
				if(res.status){
		...
		addComment(type) {
			...
			let parms = {
				action: 'add', //固定add
				pet_adoption_id: parseInt(that.id), //商品id 
				comment_id: 0, //评论主题id
			...
			uni.loading(true);
			uni.wen.util.request(uni.wen.api.ApiRootUrl + 'pets/comment/process', parms, 'POST').then(function(res) {
				uni.loading(false);
				if (res.status) {
					uni.showToast({
						title: '评论成功',
